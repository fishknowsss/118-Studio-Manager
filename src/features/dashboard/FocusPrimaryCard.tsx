    ...
    >
      ...
      {focusData ? (
        <>
          ...
          {focusData.topTasks.length > 0 ? (
            <div className="focus-highlight-tasks">
              {focusData.topTasks.map((title, i) => (
                <div key={i} className="focus-highlight-task-item">{title}</div>
              ))}
              {focusData.remainingCount > 3 ? (
                <div className="focus-highlight-task-more">+{focusData.remainingCount - 3}</div>
              ) : null}
            </div>
          ) : null}
        </>
