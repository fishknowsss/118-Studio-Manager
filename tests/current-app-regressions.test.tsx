...
describe('current app regressions', () => {
  ...
  it('keeps mobile shell changes scoped to the phone breakpoint', () => {
    ...
    expect(styleSource).toMatch(/@media \(max-width:\s*720px\)\s*\{[\s\S]*html,\s*body\s*\{[\s\S]*overflow-y:\s*auto;/)
    ...
    expect(styleSource).toMatch(/@media \(max-width:\s*720px\)\s*\{[\s\S]*\.main-content\s*\{[\s\S]*width:\s*100%;[\s\S]*min-width:\s*0;/)
    expect(styleSource).toMatch(/@media \(max-width:\s*720px\)\s*\{[\s\S]*\.main-content\s*\{[\s\S]*overflow:\s*visible;/)
    expect(styleSource).toMatch(/@media \(max-width:\s*720px\)\s*\{[\s\S]*#view-container\s*\{[\s\S]*flex:\s*none;[\s\S]*width:\s*100%;[\s\S]*overflow:\s*visible;/)
    expect(styleSource).toMatch(/@media \(max-width:\s*720px\)\s*\{[\s\S]*\.dashboard\s*\{[\s\S]*display:\s*flex;[\s\S]*width:\s*100%;[\s\S]*overflow:\s*visible;/)
    expect(styleSource).toMatch(/@media \(max-width:\s*720px\)\s*\{[\s\S]*\.dash-bottom\s*\{[\s\S]*flex-direction:\s*column;[\s\S]*overflow:\s*visible;/)
    expect(styleSource).toMatch(/@media \(max-width:\s*720px\)\s*\{[\s\S]*\.dash-bottom \.panel-body\s*\{[\s\S]*overflow:\s*visible;/)
    expect(styleSource).toMatch(/@media \(max-width:\s*720px\)\s*\{[\s\S]*\.view-body\s*\{[\s\S]*padding:\s*12px 14px calc\(var\(--mobile-nav-h\) \+ 18px\);[\s\S]*overflow:\s*visible;/)
    expect(styleSource).toMatch(/@media \(max-width:\s*720px\)\s*\{[\s\S]*\.mat-topbar\s*\{[\s\S]*flex-direction:\s*column;/)
    expect(styleSource).toMatch(/@media \(max-width:\s*720px\)\s*\{[\s\S]*\.materials-split\s*\{[\s\S]*flex-direction:\s*column;[\s\S]*overflow:\s*visible;/)
    expect(styleSource).toMatch(/@media \(max-width:\s*720px\)\s*\{[\s\S]*\.tools-view-body\s*\{[\s\S]*height:\s*auto;[\s\S]*overflow:\s*visible;/)
    expect(styleSource).toMatch(/@media \(max-width:\s*720px\)\s*\{[\s\S]*\.tool-card\s*\{[\s\S]*height:\s*auto;[\s\S]*overflow:\s*visible;/)
    expect(styleSource).toMatch(/@media \(max-width:\s*720px\)\s*\{[\s\S]*\.graph-view-body\s*\{[\s\S]*flex-direction:\s*column;/)
  ...
  it('keeps short drama interactions direct and avoids a stacked card-table layout', () => {
    ...
    const quickDialogSource = readFileSync(quickDialogSourcePath, 'utf8')
    expect(shortDramaSource).toMatch(/short-drama-board/)
    expect(shortDramaSource).toMatch(/short-drama-overview-strip/)
    expect(shortDramaSource).toMatch(/short-drama-person-strip/)
    expect(shortDramaSource).toMatch(/buildShortDramaPersonSummaries/)
    ...
    expect(shortDramaSource).toMatch(/short-drama-assignment-chips/)
    expect(shortDramaSource).toMatch(/short-drama-status-select/)
    expect(shortDramaSource).not.toMatch(/short-drama-status-actions/)
    expect(shortDramaSource).not.toMatch(/card\.allocationText \?/)
    expect(shortDramaSource).toMatch(/建小组/)
    expect(shortDramaSource).toMatch(/分配给人/)
    expect(shortDramaSource).not.toMatch(/short-drama-table/)
    expect(shortDramaSource).not.toMatch(/short-drama-stats/)
    expect(quickDialogSource).toMatch(/short-drama-person-range-list/)
    expect(quickDialogSource).toMatch(/起始集/)
    expect(quickDialogSource).toMatch(/最终集/)
    expect(quickDialogSource).not.toMatch(/id="short-drama-quick-episodes"/)
    expect(styleSource).toMatch(/\.short-drama-board/)
    expect(styleSource).toMatch(/\.short-drama-person-summary/)
    expect(styleSource).toMatch(/\.short-drama-person-range/)
