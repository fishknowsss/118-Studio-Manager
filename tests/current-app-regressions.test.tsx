...
describe('current app regressions', () => {
  ...
  it('keeps mobile shell changes scoped to the phone breakpoint', () => {
    ...
    expect(styleSource).toMatch(/@media \(max-width:\s*720px\)\s*\{[\s\S]*\.main-content\s*\{[\s\S]*overflow-y:\s*auto;/)
    expect(styleSource).toMatch(/@media \(max-width:\s*720px\)\s*\{[\s\S]*#view-container\s*\{[\s\S]*overflow-y:\s*auto;/)
    expect(styleSource).toMatch(/@media \(max-width:\s*720px\)\s*\{[\s\S]*\.dashboard\s*\{[\s\S]*display:\s*flex;[\s\S]*overflow:\s*visible;/)
